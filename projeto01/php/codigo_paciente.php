...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_token = "SELECT * FROM Paciente WHERE email='$email' AND token='$token'";
    $result_check_token = $conn->query($sql_check_token);
    
    

    if ($result_check_token->num_rows > 0) {
        echo "<script>alert('Token válido. Você pode prosseguir com a redefinição da senha.'); window.location.href = '../php/login_paciente.php';</script>";
    } else {
